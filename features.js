...
function creditRow(cells){const c=cells.map(s=>s.trim());if(c.length!==7||!/^\d{4}$/.test(c[0])||! /^[A-Za-z0-9]{7}\s*\//.test(c[1]))return null;return {semester:c[0],course:c[1],credits:/^\d+(?:\.\d+)?$/.test(c[2])?Number(c[2]):null,category:c[3]};}
function matchesCredit(r,f){return (!f.semester||r.semester===f.semester)&&(!f.category||r.category===f.category)&&(!f.query||r.course.toLocaleLowerCase().includes(f.query.trim().toLocaleLowerCase()));}
